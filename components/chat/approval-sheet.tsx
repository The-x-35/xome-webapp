...
import { ConsentLevel, type ApprovalRequest } from "@/lib/agent/tools/consent";

/** The consent sheet — shown before any alwaysAsk / first askOncePerSession tool
 *  runs. Mirrors approval_sheet.dart: tool name, args JSON, remember toggle. */
export function ApprovalSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      <div className="absolute inset-0 bg-[var(--scrim)]" onClick={() => onResolve(false, false)} />
      <div className="relative z-10 w-full max-w-md rounded-t-[var(--radius-sheet)] border border-border bg-surface p-5 shadow-[var(--shadow-card)] sm:rounded-[var(--radius-sheet)]">
        ...
        {req.consent === ConsentLevel.askOncePerSession && (
          <label className="mt-4 flex cursor-pointer items-center gap-2.5 text-[13.5px] text-text-2">
            <input
              type="checkbox"
              checked={remember}
        ...
        <div className="mt-5 flex gap-2.5">
          <Button variant="outlined" className="flex-1" onClick={() => onResolve(false, false)}>
            Decline
          </Button>
          <Button variant="filled" className="flex-1" onClick={() => onResolve(true, remember)}>
            Approve
          </Button>
        </div>
      </div>
